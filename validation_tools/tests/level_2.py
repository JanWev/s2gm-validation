...
def refl_scatter_plot(refRasterAr, valRasterAr, xlabel, ylabel, title, plot_file, xlim, ylim, group, subgroup,
                      pointsize):
    df = pd.DataFrame({'ref': refRasterAr, 'val': valRasterAr})
...
def level_2_2(test_metadata, ref_metadata, comparable, refl_bands_dict, val_name_sub_string, ref_name_sub_string,
              val_res_path):
    """
            ...
            if test_metadata['image_format'] == 'GEO_TIFF' or test_metadata['image_format'] == 'JP2':
                ...
                for i in range(len(valSubPaths)):
                    ...
                    if 'SCENE' in test_metadata['bands']:
                        band = 'SCENE'
                        if tiled_prod:
                            valBand = valSubPath + '\\' + aux_band_dict[band] + val_name_sub_string + \
                                      valSubPath.split('\\')[
                                          -1] + '.' + \
                                      file_ext
                            refBand = refSubPath + '\\' + aux_band_dict[band] + ref_name_sub_string + \
                                      refSubPath.split('\\')[
                                          -1] + '.' + \
                                      file_ext
                        else:
                            valBand = valSubPath + '\\' + aux_band_dict[band] + val_name_sub_string + test_metadata[
                                'order_name'] + '.' + \
                                      file_ext
                            refBand = refSubPath + '\\' + aux_band_dict[band] + ref_name_sub_string + ref_metadata[
                                'order_name'] + '.' + \
                                      file_ext
                        valData = gdal.Open(valBand)
                        refData = gdal.Open(refBand)

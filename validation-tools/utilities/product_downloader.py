...
def make_request(token, userid):
    ...
    if temporal_period == 'DAY':
        ...
        end_date = datetime.strptime(
            str(start_date.year) + str(
                start_date.month + 11) + str(
                calendar.monthrange(start_date.year, start_date.month)[1]),
            "%Y%m%d").date()
...
def main(TOKEN, USERID, START, RUNNING, FINISHED,
         DOWNLOAD_FOLDER):
    request_id, order_name, start_date, end_date, temporal_period, resolution = make_request(TOKEN, USERID)
    check_processing_status(TOKEN, USERID, request_id)
    downloader(START, RUNNING, FINISHED, DOWNLOAD_FOLDER, request_id, TOKEN, start_date, end_date, temporal_period, resolution, order_name)


if __name__ == '__main__':
    main(TOKEN, USERID, START, RUNNING, FINISHED,
         DOWNLOAD_FOLDER)
